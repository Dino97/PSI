...
%type <i> ternary ternary_exp md_exp
	...
	: _LPAREN rel_exp _RPAREN _QMARK ternary_exp _COLON ternary_exp
		{
			...
			code("\n\t\t\t%s\t@true%d", get_jump_stmt($2, FALSE), ++lab_num);
	...
	: _ID
		{
			int idx = lookup_symbol($1, (VAR|PAR));
			if(idx == -1)
          err("invalid lvalue '%s' in ternary operator", $1);
...
if_part
  : _IF _LPAREN
    ...
    rel_exp
      {
        code("\n\t\t\t%s\t@false%d", get_jump_stmt($4, TRUE), $<i>3);
        gen_snlab("true", $<i>3);
      }
    _RPAREN statement
      {
